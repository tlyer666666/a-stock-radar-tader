...
type NormalizedSettings = Required<Settings>;
...
const defaultBacktestStartDate = () => {
  const date = new Date();
...
const riskProfilePresets: Array<{
...
const sameStrategySet = (left: string[] = [], right: string[] = []) =>
...
const shanghaiDateTag = () => new Intl.DateTimeFormat("en-CA", {
...
const normalizeRiskProfile = (value: unknown): Settings["riskProfile"] =>
  value === "conservative" || value === "aggressive" ? value : "balanced";

const normalizeSettings = (input: Settings): NormalizedSettings => {
  ...
  return {
    ...
    maxDailyRiskPercent: clampNumber(
      merged.maxDailyRiskPercent ?? 3.2,
      0.3,
      12,
      3.2
    ),
...
const riskProfileLabel = (value: Settings["riskProfile"]) =>
  value === "conservative" ? "稳健保守" : value === "aggressive" ? "积极进取" : "平衡稳健";

const buildSettingsByRiskProfile = (input: Settings, profile: Settings["riskProfile"]): Settings => {
...
function SectorView({ rows, loading, loadError, onRefresh, onOpenStock }: {
  ...
  useEffect(() => {
    ...
    sectorSearchTimer.current = window.setTimeout(async () => {
      ...
      try {
        setSectorSuggestions(await window.stockApi.searchSectors(sectorQuery));
      } catch {
        setSectorSuggestions([]);
      } finally {
        setSectorSearching(false);
      }
    }, 220);
    return () => window.clearTimeout(sectorSearchTimer.current);
  }, [sectorQuery]);
        ...
        <>
          {customSector && (
            <div className="sector-query-result">
              ...
              <button onClick={() => { setCustomSector(null); setSectorQuery(""); }}>返回涨停板块榜</button>
            </div>
          )}
          {sector.partial && (
            ...
            <div className="panel sector-board-panel">
              ...
              <div className="sector-comparison-table">
                ...
                {normalizedRows.map((item) => {
                  ...
                  return (
                  <button className={`sector-row ${!customSector && sector.name === item.name ? "active" : ""}`} key={item.name} onClick={() => { setCustomSector(null); setSelectedName(item.name); }}>
...
function SettingsView({ value, onSave }: { value: Settings; onSave: (next: Settings) => void }) {
  ...
  const test = async () => {
    ...
    setTestResult(null);
    try {
      setTestResult(await window.stockApi.testProvider(draft));
    } catch (error) {
      setTestResult({
        ok: false,
      ...
      <div className="settings-grid">
        <div className="panel settings-card">
          ...
          <div className="settings-actions">
            ...
            <button className="primary-btn" onClick={saveDraft}><ShieldCheck size={17} />保存设置</button>
          </div>
        ...
        <div className="panel settings-card">
          ...
          <div className="settings-actions">
            <button className="secondary-btn" onClick={saveDraft}><ShieldCheck size={17} />保存执行参数</button>
          </div>
        </div>
        ...
        <div className="panel settings-card strategy-config-card">
          ...
          <div className="settings-actions">
            <button className="primary-btn" onClick={saveDraft}><ShieldCheck size={17} />保存策略组合</button>
          </div>
        </div>
      </div>
    </>
